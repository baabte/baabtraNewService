...
@csrf_exempt
@api_view(['GET','POST'])
def SaveExistingElementView(request):  #this service will add & update course elements
    #connect to our local mongodb
    db = Connection(settings.MONGO_SERVER_ADDR,settings.MONGO_PORT)
    #get a connection to our database
    dbconn = db[settings.MONGO_DB]
    ...
    if request.method == 'POST':
        try:
            stream = StringIO(request.body)
            data = JSONParser().parse(stream)
            coureDetails=dbconn.system_js.fnSaveExistingElement(data);
        except ValueError:
            return Response(json.dumps(ValueError, default=json_util.default))
        return Response(json.dumps(coureDetails, default=json_util.default))
    else:        
...
@csrf_exempt
@api_view(['GET','POST'])
def moveCourseElementView(request):  #this service will add & update course elements
    #connect to our local mongodb
    db = Connection(settings.MONGO_SERVER_ADDR,settings.MONGO_PORT)
    #get a connection to our database
    dbconn = db[settings.MONGO_DB]
    ...
    if request.method == 'POST':
        try:
            stream = StringIO(request.body)
            data = JSONParser().parse(stream)
            coureDetails=dbconn.system_js.fnMoveCourseElement(data['moveObject']);
        except ValueError:
            return Response(json.dumps(ValueError, default=json_util.default))
        return Response(json.dumps(coureDetails, default=json_util.default))
    else:        
...
def saveSubjectView(request):  #this service will add & update Subject
    ...
    if request.method == 'POST':
        try:
            stream = StringIO(request.body)
            data = JSONParser().parse(stream)
            courses = dbconn.system_js.fnSaveSubject(data);
        except ValueError:
            return Response(json.dumps(ValueError, default=json_util.default))
        return Response(json.dumps(courses, default=json_util.default))
    else:        
...
@csrf_exempt
@api_view(['GET','POST'])
def loadSubjectView(request):  #this service will load Subject
    #connect to our local mongodb
    db = Connection(settings.MONGO_SERVER_ADDR,settings.MONGO_PORT)
    #get a connection to our database
    dbconn = db[settings.MONGO_DB]
    ...
    if request.method == 'POST':
        try:
            stream = StringIO(request.body)
            data = JSONParser().parse(stream)
            courses = dbconn.system_js.fnLoadSubject(data);
        except ValueError:
            return Response(json.dumps(ValueError, default=json_util.default))
        return Response(json.dumps(courses, default=json_util.default))
    else:        
